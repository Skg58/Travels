...
const Page = () => {
  ...
  const totalAmount =
    passengers.length *
      (selectedLocationObj
        ? selectedLocationObj.price + selectedPackObj?.price
        : 0) -
    discount;
  ...
  const CanAddPassenger = () => {
    return passengers.every(
      (p) => p.name.trim() !== "" && p.age > 0 && p.phone.trim() !== ""
    );
  };
  const handleDelete = (index) => {
    const newarr = passengers.filter((_, i) => {
      return i !== index;
    });
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        try {
          ...
          console.error(err);
          console.log(
            "❌ Payment successful, but something went wrong while saving data."
          );
        }
        const queryString = new URLSearchParams(details);
        ...
        <div className="flex-1 p-8 flex flex-col justify-center w-[70vw]">
          ...
          {step === 1 && (
            ...
            >
                ...
                {packs.map((pack) => (
                  ...
                  >
                    ...
                    <div className="font-semibold flex items-center gap-2">
                      {pack.name}
                      {pack.id === "Pro" && (
                        <span className="ml-1 px-2  py-0.5 bg-yellow-400 text-[9px] font-bold rounded-full text-yellow-900 shadow border border-yellow-300 animate-pulse">
                          Most Bought
                        </span>
                      )}
                    </div>
              ...
              <Link href={"/Packages"}>
                <div className="text-sm mt-1.5 underline text-blue-600 font-bold">
                  Know about Package
                </div>
              </Link>
              <button
                type="submit"
                className="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 mt-4 cursor-pointer"
              >
                Next
              </button>
            </form>
          ...
          {step === 2 && (
            ...
            >
              <label className="block mb-4 text-gray-700 font-semibold text-lg">
                Passenger Details
              </label>
              <div className="space-y-6 max-h-44  overflow-y-auto ">
                {passengers.map((p, idx) => (
                  <div
                    key={idx}
                    className="bg-indigo-50 rounded-lg p-2 flex items-center gap-5 shadow-sm"
                  >
                    ...
                    <div className="flex items-center gap-3 flex-col md:flex-row">
                      <div>
                        ...
                        <input
                          ...
                          required
                          className="border rounded px-3 py-2 w-[40vw] md:w-[6vw]"
                        />
                      </div>
                      <div>
                        {" "}
                        <input
                          ...
                          required
                          min={1}
                          className="border rounded px-3 w-[40vw] md:w-[5vw] py-2 "
                        />
                      </div>
                      <div>
                        {" "}
                        <input
                          ...
                          onChange={(e) => handlePassengerChange(idx, e)}
                          required
                          className="border rounded px-3 py-2 w-[40vw] md:w-[8vw]"
                        />
                      </div>
                      <div
                        onClick={() => {
              ...
              <button
                ...
                className="mt-4 mb-6 bg-green-200 text-green-700 px-4 py-2 rounded hover:bg-green-300 cursor-pointer disabled:bg-green-100 "
              >
                + Add Passenger
              </button>
              <div className="flex justify-between">
                <button
                  type="button"
                  onClick={handleBack}
                  className="bg-gray-200 text-gray-700 py-2 px-6 rounded hover:bg-gray-300 cursor-pointer"
                >
                  Back
                </button>
                <button
          ...
          {step === 3 && (
            <form className="animate-fadeIn">
              ...
              <div className="bg-indigo-50 rounded-lg p-4 md:p-6 mb-6 shadow">
                <div>
                  <span className="md:font-semibold text-indigo-700">
                    Date:
                  </span>{" "}
                  {travelDate}
                </div>
                <div>
                  ...
                  {selectedLocation} (₹{selectedLocationObj?.price}/person)
                </div>
                <div>
                  <span className="md:font-semibold text-indigo-700">
                    Package:
                  </span>{" "}
                  {selectedPackObj?.name}(₹{selectedPackObj?.price}/person)
                </div>
                <div>
                  <span className="md:font-semibold text-indigo-700">
                    Passengers:
                ...
                <div className="mt-4 text-lg font-bold text-indigo-800">
                  Total: ₹{totalAmount}
                </div>
              </div>
              ...
              <div className="mb-4">
                {!showCouponInput ? (
                  ...
                  <div className="mt-4">
                    <label className="font-semibold text-indigo-700 block mb-1">
                      Enter Coupon Code:
                    </label>
                    ...
                    {discount !== 0 && (
                      <div className="mt-2  text-sm md:text-lg font-bold text-green-700">
              ...
              <div className="flex justify-between">
                ...
                <button
                  ...
                  className="bg-green-600 text-white py-2 px-3 md:px-6 rounded hover:bg-green-700 font-semibold cursor-pointer"
                >
                  Proceed to Pay
                </button>
              </div>
        ...
        <div className="hidden md:flex flex-col justify-between bg-gradient-to-br from-gray-600 to-blue-300 text-white w-96 p-8">
          <div>
            ...
            <div className="bg-white/20 rounded-lg p-4 mb-4">
              ...
              <p>
                Get <span className="font-bold">10% OFF</span> on your first
                booking!
              </p>
              <p>
                Use Code :
                <span className="text-gray-950 font-bold"> "FIRST"</span>
              </p>
            </div>
          </div>
          <Image
