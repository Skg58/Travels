     ...
     <Link href={"/"}> <div className='text-3xl flex flex-col gap-5 ' >
      ...
      {session ? (
        <div className='mt-[30px] min-w-[6.3vw]   text-white opacity-[0.8] hover:opacity-[1] cursor-pointer rounded-full flex items-center justify-end'>
          <DropdownMenu>
            <DropdownMenuTrigger><div className=' p-1 rounded-full mr-1 bg-black '><img src={session.user.image ? session.user.image : session.user.picture} width={35} className='rounded-full cursor-pointer ' alt="img" /></div></DropdownMenuTrigger>
            <DropdownMenuContent>
